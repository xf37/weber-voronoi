...
path0 = "W:\\data\\upload\\application 1\\"
...
inFeatures = []
Extent = "4084198.27056578 5344393.21032053 4120113.30106668 5379391.68822291"
resouce =   "RM"
...
##########################################################################
# Process: Create Thiessen Polygons based on layers of resouce points (except
# markets)
##########################################################################
start2 = time.time()
for i in range(1, n_rsc):
    ...
    arcpy.CreateThiessenPolygons_analysis(path0 + resouce + str(i) + ".shp",
...
##########################################################################
# Process: Intersect voronoi layers of resource points and boundary
##########################################################################
start3 = time.time()
...
for i in range(1, n_rsc):
     coo[i] = []
     table = DBF(path0 + resouce + str(i) + '.dbf', load = True)
     length = len(table)
...
##########################################################################
# save total weighted distance and get optimal solution
##########################################################################
...
def writeResult(n_pts, n_rsc, n_mkts, n_test, end0, end1, end2, end3, n_iteration, min_twd, min_index, n_polygons):
    ...
    file.write('the total number of polygons is: ' + str(n_polygons) + '\n')
